...
public class DownloadExcelUtil {
     ...
     * 通过Excel的形式导出数据
     * @param fileName 文件名
     * @param sheetName 工作簿名称，如果其值为空，则默认为fileName的值
     * @param datas 数据，其中list中第一条数据应为每一列的标题数据
     * @throws IOException
     */
    public static void exportDataByExcel(String fileName, String sheetName, List<List<String>> datas)
            throws IOException {
        ...
        if (response != null && response instanceof HttpServletResponse) {
            ...
            for(int i=0;i<rowNum;i++){
                ...
                List<String> date = datas.get(i);
                for(int j=0;j<date.size();j++){
                    row.createCell(j).setCellValue(date.get(j));
                }
            }
        ...
        try {
            if (o instanceof ArrayList<?>) {
                ...
                if (o == null) {
                    ...
                    wsheet.addCell(new DateTime(col, row, DateUtil.stringToDate(o
                        .toString()), wcfFC));
                } else {
                    wsheet.addCell(new Label(col, row, o.toString(), wcfFC));
                }
