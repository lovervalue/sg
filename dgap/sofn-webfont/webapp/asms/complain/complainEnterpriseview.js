 ...
 */
var myapp=angular.module("myapp",[]).controller("complainEnterpriseview",function($scope,$http,$stateParams, $state){
    $scope.subjEntId = window.localStorage.getItem("entityIdcode");
    $scope.subjEnt={};
    //获取主体信息
    $http({
        headers:{
    ...
    $scope.removeLocalStorage=function(){
        var goBackUrl = window.localStorage.getItem("comComplainEnterUrl");
        if (goBackUrl) {
            window.localStorage.removeItem("comComplainEnterUrl");
