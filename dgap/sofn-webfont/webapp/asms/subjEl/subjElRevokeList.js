...
myapp.controller("subjElRevokeList",function($scope,$http,$state){
    //判定页面权限、tab页、按钮
    ...
    a.map(function(item,i){
        ...
        }else if(item.text=="执法机构主体信息"){
            $(".subjElList").css("display","inline-block");
        }
    });
    $scope.subjElRevokeListQueryParams = JSON.parse(window.sessionStorage.getItem("subjElRevokeListQueryParams"));
    //获取用户信息
    $scope.user = {};
    $scope.organization = {};
    //变更待审核数量
    ...
    if(window.localStorage.getItem("asmsAllSysDictAndUserInfo")){
        ...
        if($scope.subjElRevokeListQueryParams){
            $("input[name='date']").val($scope.subjElRevokeListQueryParams.date);
            $("input[name='elName']").val($scope.subjElRevokeListQueryParams.elName);
            $("input[name='auditState']").val($scope.subjElRevokeListQueryParams.auditState);
            $.fn.mycity("sheng", "shi", "xian", $scope.subjElRevokeListQueryParams.elAreaId);
        }
        loadGrid();
    }else{
    ...
    //执法机构主体撤销待审核
    function loadGrid() {
        $("#elRevokeGrid").dataTable({
            ajax: {
                url: "/sofn-asms-web/subjEnforceLaw/getSubjElRevokeList",
                ...
                data: function (params) {
                    var subjElRevokeListQueryParams = JSON.parse(window.sessionStorage.getItem("subjElRevokeListQueryParams"));
                    if(subjElRevokeListQueryParams){
                        params.elAreaId = subjElRevokeListQueryParams.elAreaId;
                        params.elName = subjElRevokeListQueryParams.elName;
                        params.date = subjElRevokeListQueryParams.date;
                        params.auditState = subjElRevokeListQueryParams.auditState;
                    }else{
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) { // 模板化列显示内容
                        if($scope.iswrite==2){
                            if (row.AUDITSTATE != "0") {
                                return '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="subjElRevokeAuditshow(\'' + data + '\')">查看</a>';//转义传值
                            } else {
                                return '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="subjElRevokeAudit(\'' + data + '\')">审核</a>';//转义传值
                            }
                        }
                    }
                }
    ...
    //重新加载表格
    $scope.querySubjElRevoke=function(){
        ...
        var subjElRevokeListQueryParams = {};
        subjElRevokeListQueryParams.auditState = $("select[name='auditState']").val();
        subjElRevokeListQueryParams.elAreaId = $scope.getAreaBySelect();
        subjElRevokeListQueryParams.elName = $("input[name='elName']").val();
        subjElRevokeListQueryParams.date = $("input[name='date']").val();
        window.sessionStorage.setItem("subjElRevokeListQueryParams",JSON.stringify(subjElRevokeListQueryParams));
        $("#elRevokeGrid").dataTable().api().ajax.reload();
    }

    window.subjElRevokeAuditshow=function(data){
        window.localStorage.setItem("subjElRevokeId",data);
        $state.go("index.content.asms/subjEl/subjElRevokeAudit");
    }
    //跳转撤销审核页面
    window.subjElRevokeAudit=function(data){
        if($scope.iswrite!="2"){
