...
myapp.controller("subjElCancelList",function($scope,$http,$state){
//判定页面权限、tab页、按钮
    ...
    $scope.subjElCancelListQueryParams = JSON.parse(window.sessionStorage.getItem("subjElCancelListQueryParams"));
    //获取用户信息
    $scope.user = {};
    $scope.organization = {};
    //变更待审核数量
    ...
    if(window.localStorage.getItem("asmsAllSysDictAndUserInfo")){
        ...
        if($scope.subjElCancelListQueryParams){
            $("input[name='date']").val($scope.subjElCancelListQueryParams.date);
            $("input[name='elName']").val($scope.subjElCancelListQueryParams.elName);
            $("input[name='auditState']").val($scope.subjElCancelListQueryParams.auditState);
            $.fn.mycity("sheng", "shi", "xian", $scope.subjElCancelListQueryParams.elAreaId);
        }
        loadGrid();
    }else{
    ...
    //执法机构主体注销待审核
    function loadGrid() {
        $("#elCancelGrid").dataTable({
            ajax: {
                url: "/sofn-asms-web/subjEnforceLaw/getSubjElCancelList",
                ...
                data: function (params) {
                    ...
                    var subjElCancelListQueryParams = JSON.parse(window.sessionStorage.getItem("subjElCancelListQueryParams"));
                    if(subjElCancelListQueryParams){
                        params.elAreaId = subjElCancelListQueryParams.elAreaId;
                        params.elName = subjElCancelListQueryParams.elName;
                        params.date = subjElCancelListQueryParams.date;
                        params.auditState = subjElCancelListQueryParams.auditState;
                    }else{
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) { // 模板化列显示内容
                        if ($scope.iswrite==2){
                            if (row.AUDITSTATE != "0") {
                                return '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="subjElCancelAuditshow(\'' + data + '\')">查看</a>';//转义传值
                            } else {
                                return '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="subjElCancelAudit(\'' + data + '\')">审核</a>';//转义传值
                            }
                        }
                    }
                },
    ...
    //重新加载表格
    $scope.querySubjElCancel=function(){
        ...
        var subjElCancelListQueryParams = {};
        subjElCancelListQueryParams.auditState = $("select[name='auditState']").val();
        subjElCancelListQueryParams.elAreaId = $scope.getAreaBySelect();
        subjElCancelListQueryParams.elName = $("input[name='elName']").val();
        subjElCancelListQueryParams.date = $("input[name='date']").val();
        window.sessionStorage.setItem("subjElCancelListQueryParams",JSON.stringify(subjElCancelListQueryParams));
        $("#elCancelGrid").dataTable().api().ajax.reload();
    }

    window.subjElCancelAuditshow=function(data){
        window.localStorage.setItem("subjElCancelId",data);
        $state.go("index.content.asms/subjEl/subjElCancelAudit");
    }
    //跳转注销审核页面

    window.subjElCancelAudit=function(data){
        if($scope.iswrite!="2"){
