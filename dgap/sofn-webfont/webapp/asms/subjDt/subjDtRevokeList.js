...
myapp.controller("subjDtRevokeList",function($scope,$http,$state){
//判定页面权限、tab页、按钮
    ...
    $scope.subjDtRevokeListQueryParams = JSON.parse(window.sessionStorage.getItem("subjDtRevokeListQueryParams"));
    //变更待审核数量
    ...
    if(window.localStorage.getItem("asmsAllSysDictAndUserInfo")){
        ...
        if($scope.subjDtRevokeListQueryParams){
            $("input[name='date']").val($scope.subjDtRevokeListQueryParams.date);
            $("input[name='dtName']").val($scope.subjDtRevokeListQueryParams.dtName);
            $("input[name='auditState']").val($scope.subjDtRevokeListQueryParams.auditState);
            $.fn.mycity("sheng", "shi", "xian", $scope.subjDtRevokeListQueryParams.dtAreaId);
        }
        loadGrid();
    }else{
    ...
    //检测机构主体撤销待审核
    function loadGrid() {
        $("#dtRevokeGrid").dataTable({
            ajax: {
                url: "/sofn-asms-web/subjDetection/getSubjDtRevokeList",
                headers: {
                    token: window.localStorage.getItem("token")
                },
                ...
                data: function (params) {
                    ...
                    var subjDtRevokeListQueryParams = JSON.parse(window.sessionStorage.getItem("subjDtRevokeListQueryParams"));
                    if(subjDtRevokeListQueryParams){
                        params.dtAreaId = subjDtRevokeListQueryParams.dtAreaId;
                        params.dtName = subjDtRevokeListQueryParams.dtName;
                        params.date = subjDtRevokeListQueryParams.date;
                        params.auditState = subjDtRevokeListQueryParams.auditState;
                    }else{
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) { // 模板化列显示内容
                        if($scope.iswrite==2){
                            if (row.AUDITSTATE != "0") {
                                return '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="subjDtRevokeAuditshow(\'' + data + '\')">查看</a>';//转义传值
                            } else {
                                return '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="subjDtRevokeAudit(\'' + data + '\')">审核</a>';//转义传值
                            }
                        }
                    }
                }
    ...
    //重新加载表格
    $scope.querySubjDtRevoke=function(){
        //点击间隔
        $scope.btnDisabled = true;
        setTimeout(function(){
            $scope.$apply($scope.btnDisabled = false);
        }, 500);
        var subjDtRevokeListQueryParams = {};
        subjDtRevokeListQueryParams.auditState = $("select[name='auditState']").val();
        subjDtRevokeListQueryParams.dtAreaId = $scope.getAreaBySelect();
        subjDtRevokeListQueryParams.dtName = $("input[name='dtName']").val();
        subjDtRevokeListQueryParams.date = $("input[name='date']").val();
        window.sessionStorage.setItem("subjDtRevokeListQueryParams",JSON.stringify(subjDtRevokeListQueryParams));
        $("#dtRevokeGrid").dataTable().api().ajax.reload();
    }
    window.subjDtRevokeAuditshow=function(data){
        window.localStorage.setItem("subjDtRevokeId",data);
        $state.go("index.content.asms/subjDt/subjDtRevokeAudit");
    }
    //跳转撤销审核页面
    window.subjDtRevokeAudit=function(data){
        if($scope.iswrite!="2"){
