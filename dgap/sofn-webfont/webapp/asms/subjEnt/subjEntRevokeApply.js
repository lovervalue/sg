var myapp=angular.module("myapp",[]).controller("subjEntRevokeApply",function($scope,$http,$state){
            ...
            columns: [
                ...
                },
                {
                    ...
                    render:function (data){
                        if (data) {
                            var arr = data.split(";");
                            var text = ["营业执照变更为", "组织机构代码照变更为", "身份证正面照变更为", "身份证反面照变更为", "手持身份证照变更为"];
    ...
    //撤销生产营业主体申请
    $scope.applySubjEntRevoke=function(){
        //获取区域id
        $scope.subjEnt.area = $scope.areaId;
        //获取entityId,必传,将主体id赋值给entityId
        $scope.subjEnt.entityId = $scope.subjEnt.id;
        ...
        $http({
            ...
            url:"/sofn-asms-web/subjEnt/addSubjEntRevokeApply",
            method:"post",
            data:$scope.subjEnt
        }).success(function(data){
