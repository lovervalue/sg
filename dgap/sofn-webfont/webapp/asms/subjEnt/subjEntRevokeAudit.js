var myapp=angular.module("myapp",[]).controller("subjEntRevokeAudit",function($scope,$http,$state){

    //获取本地存储的ID
    $scope.subjEntRevokeId=window.localStorage.getItem("subjEntRevokeId");
    ...
    $http({
        ...
        method:"post",
        data:{id:$scope.subjEntRevokeId}
    }).success(function(data){
        $scope.subjEntRevoke=data.subjEntChange;
        $scope.subjEnt=$scope.subjEntRevoke;
        $scope.subjEnt.area=$.fn.Cityname($scope.subjEnt.area);
        if($scope.subjEntRevoke.approveStatus=="1"||$scope.subjEntRevoke.approveStatus=="2"){
            $("textarea[name='approveOpinion']").attr("readonly","readonly");
        }else{
            //待审核状态是审核意见默认为空
            $scope.subjEntRevoke.approveOpinion="";
    ...
    $scope.removeLocalStorage=function(){
        window.localStorage.removeItem("subjEntRevokeId");
        $state.go("index.content.asms/subjEnt/subjEntRevokeList");
    }

    //审核生产经营主体撤销申请
    $scope.auditSubjEntRevoke=function(auditState){
        if (!$("#revokeForm").data('bootstrapValidator').validate().isValid()) {
            return false;
        }//必须有
        $scope.subjEntRevoke.approveStatus=auditState;
        $scope.subjEntRevoke.approveType="3";//代表变更待审核-1-注销-2-变更
        $scope.subjEntRevoke.approveOpinion=$("textarea[name='approveOpinion']").val();//审核意见
    ...
    $scope.openModal = function (event) {
        ...
        $http.post('/sofn-asms-web/subjEnt/findSubjEntAuthenticationList',$.param({id:$scope.subjEntRevoke.entityId, type: type}), {
            headers: {
