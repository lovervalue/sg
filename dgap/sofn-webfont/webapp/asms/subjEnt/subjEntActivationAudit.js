var myapp=angular.module("myapp",[]).controller("subjEntActivationAudit",function($scope,$http,$state){

    //获取本地存储的ID
    $scope.subjEntActivationId=window.localStorage.getItem("subjEntActivationId");
    $('#registerForm').bootstrapValidator();//初始化验证，必须有
    ...
    $http({
        ...
        url:"/sofn-asms-web/subjEnt/findSubjEntActivateById",
        method:"post",
        params:{id:$scope.subjEntActivationId}
    }).success(function(data){
        $scope.subjEnt=data.data.ttsScltxxcjRegiter;
        $scope.subjEnt.area=$.fn.Cityname(data.data.ttsScltxxcjRegiter.area);
        if($scope.subjEnt.approveStatus=="1"||$scope.subjEnt.approveStatus=="2"){
            $("textarea[name='approveOpinion']").attr("readonly","readonly");
        }else{
            //待审核状态下审核意见默认为空
            //$("textarea[name='approveOpinion']").val("");
            $scope.subjEnt.approveOpinion="";
            $("#passBut").css("display","inline-block");
            $("#refuseBut").css("display","inline-block");
        }
        //年产量是否显示
        if($scope.subjEnt.annualOutput==null || $scope.subjEnt.annualOutput.trim()=="" ||$scope.subjEnt.annualOutput==undefined){
            $("#zzOutput").hide();
            $("#zzOutput02").hide();
            $("#zzOutput03").hide();
        }
        if($scope.subjEnt.annualOutputX==null || $scope.subjEnt.annualOutputX.trim()=="" ||$scope.subjEnt.annualOutputX==undefined){
            $("#xmOutput").hide();
            $("#xmOutput02").hide();
            $("#xmOutput03").hide();
        }
        if($scope.subjEnt.annualOutputS==null || $scope.subjEnt.annualOutputS.trim()=="" ||$scope.subjEnt.annualOutputS==undefined){
            $("#scOutput").hide();
            $("#scOutput02").hide();
            $("#scOutput03").hide();
        }
        //营业期限是否长期
        if($scope.subjEnt.isLong=='true'){//长期
            $scope.subjEnt.isLong='长期';
            $("#isNotLong").hide();
            $("#isLong").show();
        }else{
            //非长期
    ...
    $scope.removeLocalStorage=function(){
        window.localStorage.removeItem("subjEntActivationId");
        $state.go("index.content.asms/subjEnt/subjEntActivationList");
    }

    //审核生产经营主体备案申请
    $scope.auditSubjEntActivation=function(auditState){
        if (!$("#registerForm").data('bootstrapValidator').validate().isValid()) {
            return false;
        }//必须有
        $scope.subjEnt.approveStatus=auditState;
        //营业时间长期
        if( $scope.subjEnt.isLong=='长期'){
            $scope.subjEnt.isLong='true';
        }
        //审核意见
        $scope.subjEnt.approveOpinion=$("textarea[name='approveOpinion']").val();
        $("button").attr("disabled",true);
        $http({
            headers:{
                token:window.localStorage.getItem("token")
            },
            url:"/sofn-asms-web/subjEnt/reviewSubjEntActivate",
            method:"post",
            params:{
                "id": $scope.subjEntActivationId,
                "status":auditState,
                "suggestion": $("textarea[name='approveOpinion']").val()
            }
        }).success(function(data){
