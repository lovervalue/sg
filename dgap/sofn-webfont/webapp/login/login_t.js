...
myapp.controller("login", function($scope, $http, $state, $cookies, $location) {
    ...
    if ($location.search() && $location.search().pf) {
        ...
        $scope.ptName = pfs[$location.search().pf];
    }
    ...
    $scope.toggleTab = function(index) {
        ...
        $('.login-wrap').find('.section-wapper').css({
    ...
    $scope.flushValidateCode = function() {
            ...
            $http({
                ...
                //     method: "get"
                // }).success(function(data) {
                $scope.codeImageSrc = "/sofn-sys-web/generatorImage?time=" + now + "&curr=" + $scope.prevUUID;
                // });
            })
        }
        //获取公钥
    ...
    $scope.login = function() {
        ...
        $scope.disabled = true;
        //需要加密的业务数据
        var data = { account: $scope.account, password: $scope.password, uuid: $scope.prevUUID, code: $scope.code };
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.cookie("account", $scope.account, { expires: 15 }); // 存储一个带7天期限的 cookie
                if (data.user) {
                    $.cookie("userName", data.user.userName, { expires: 15 }); // 存储一个带7天期限的 cookie
                } else {
                    $.cookie("userName", $scope.account, { expires: 15 }); // 存储一个带7天期限的 cookie
                }
                if ($scope.rememberPwd) {
                    $.cookie("rememberPassword", +$scope.rememberPwd);
                    $.cookie("password", $scope.password, { expires: 15 }); // 存储一个带7天期限的 cookie
                } else {
                    $.cookie("rememberPassword", '', { expires: -1 });
                    $.cookie("password", $scope.password, { expires: -1 }); // 存储一个带7天期限的 cookie
                }
                //des随机key解密token
                var decryptText = decryptDES(data.token, desKey);
    ...
    $scope.ukLogin = function() {
        if (!verForm() || $scope.ukdisabled) {
            return;
        }
        // $scope.ukdisabled = true;
        jBox.tip('请插入usb key', 'info');
    };
})
