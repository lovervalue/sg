...
var radioId ="";
var myapp = angular.module("myapp", []).controller("enterprise", function ($scope, $http, $state) {
    $(document)
        .off('focus.date')
        .on('focus.date', '.Wdate', function () {
            console.log($(this).data().$ngModelController.$rootScope);
            // $scope = $(this).data().$ngModelController;
            var hasNext = $(this).next('input').hasClass('Wdate');
            var hasPrev = $(this).prev('input').hasClass('Wdate');
            ...
            var currentModel = $scope;
            // console.log('configs', configs);
            models.map(function (item, index) {
                currentModel =  index < models.length - 1 ?  $scope[item] : currentModel;
            });
    ...
    $scope.enterpriseNextStep = function () {
        ...
        if($scope.flag == true){
            jBox.tip("此企业工商注册号有误,请核实重新填写!")
            return false;
        }
        //验证那个注册号是否重复
        if (isExist == "Y") {
            jBox.tip("此企业工商注册号已经存在,请核实重新填写!")
            return false;
        }
        //图片信息验证
