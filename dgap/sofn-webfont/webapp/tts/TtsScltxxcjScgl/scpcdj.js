var myapp = angular.module("myapp", []).controller("scpcdj", function($scope, $http, $state) {
    ...
    $scope.addTtsScltxxcjScgl = function() {
            ...
            if (!$("input[name='harvestTime']").val()) {
                ...
                jBox.tip("选择的收获时间必须小于当前时间！", 'info');
                return;
            }
            if (!$("input[name='productAmount']").val()) {
                jBox.tip("请输入收获数量，只能输入1~99999之间的数字！", 'info');
                return;
