...
myapp.controller("slaugterSalesRsxs",function($scope,$http,$state){
...
//            sScrollY : "auto",
                ...
                },
                {
                    ...
                    render : function(data, type, row) {
                        ...
                        return "<button class='btn btn-primary btn-xs'" +
                            "onclick=selectCustomer('"+row.USER_NAME+"','"+row.PHONE+"','"+row.NAME+"','"+row.ADDRESS+"')>" +
                            "选择客户</button>";
                    }
                }
            ],
     ...
     */
    window.selectCustomer = function(uname,phone,name,address){
        $("#ztmc").val(name);
        $("#lxdh").val(phone);
        $("#lxr").val(uname);
        $("#ztdz").val(address);
    ...
    $scope.subCpxs = function(){
        ...
        for( i=0;i<rows.length;i++){
            ...
            var td_cppc = $("<td>").html(rows[i].SLAUGHTER_BATCH_NO).appendTo(tr);
            var td_cpsl = $("<td>").html(count).appendTo(tr);
            var td_cpdw = $("<td>").html(rows[i].PRODUCT_UNIT).appendTo(tr);
        }
    ...
    $scope.toCpxs = function(){
        ...
        if(xsRows.length > 0){
            for(var i=0;i<xsRows.length;i++) {
                ...
                var productType = xsRows[i].PRODUCT_TYPE;
        ...
        var lxr = $("#lxr").val();
        var ztdz = $("#ztdz").val();
        var jsonCustomer = JSON.stringify({
            name:ztmc,
            userName:lxr,
            phone:lxdh,
            address:ztdz
        ...
        $.post("/sofn-tts-web/ttsScltxxcjSlaSale/addXsdjrs",config,
            function(data){
