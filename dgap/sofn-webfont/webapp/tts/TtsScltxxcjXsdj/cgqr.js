...
myapp.controller("cgqr",function($scope,$http,$state){
    ...
    $('#reasonForm').bootstrapValidator();//初始化表单验证
    /**
     * 分页展示数据
     */
    $(function() {
            ...
            columns : [
                ...
                },
                {
                    ...
                    render : function(data, type, row) {
                        ...
                        var xsqr_a = '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="findXsxq(\''+data+'\',\''+userIdCode+'\')">确认收货</a>';
                        var xsth_a = '<a style="margin-right: 10px;cursor: pointer" class="Check-report color"  onclick="findXsth(\''+data+'\')">采购退回</a>';
    ...
    $("#btn_cgth").click(function(){
        $('#reasonForm').bootstrapValidator();//初始化表单验证
        //表单校验
        if (!$("#reasonForm").data('bootstrapValidator').validate().isValid()) {
            return false;
        }
        var id = $("#thid").val();
        ...
        $.ajax({
            url:"/sofn-tts-web/ttsScltxxcjCggl/updateTtsScltxxcjCgth",
            type:'POST',//POST方式
        ...
        /*$.post("/sofn-tts-web/ttsScltxxcjCggl/updateTtsScltxxcjCgth",config,function(data){
            $("#xsthyy").val("");
            $("#grid").dataTable().api().ajax.reload();
        });*/
    ...
    $("#btn_cgqr").click(function(){
        var id = $("#xsid").val();
        var customerUserId = $("#s_UserIdcode").text();
        $.ajax({
            url:"/sofn-tts-web/ttsScltxxcjCggl/updateTtsScltxxcjCgqr",
            type:'POST',//POST方式
            dataType:'json',//返回json类型
            data:{
                id : id,
