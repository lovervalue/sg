...
myapp.controller("routineMonitorView",function($scope,$http,$state){
        ...
        $.ajax({
            ...
            success:function(datax){
                if(datax.httpCode == 200){
                    var datas = datax.data;
                    var Row = '<option value="">全部</option> ';
                    if (datas.length > 0){
                        for (var i = 1; i <=datas.length; i++){
                            var td = datas[i-1];
                            Row =  Row+'<option>'+td.taskName+'</option> ';
                        }
                    }
                    $("#taskName").html(Row);
                }else{
        ...
        $("#grid").dataTable({
            ajax : {
                ...
                data : function(params) {
                    ...
                    params.startTime = $("input[name='startTime']").val();//检测时间
            ...
            columns : [
                ...
                },
                {
                    ...
                    render : function(data, type, row,meta) { // 模板化列显示内容
                        if (data!=null){
                            return '<input class="gui-input" readonly="readonly" value="'+data+'" type="text"/>';
                        }else {
                            data = "";
                            return '<input class="gui-input" readonly="readonly" value="'+data+'" type="text"/>';
                        }
                        ...
                        if (v == true)
                            $http({
                                ...
                                if (blob.size > 0) {
                                    ...
                                    a.setAttribute('style', 'display:none');
                                    a.setAttribute('href', objectUrl);
                                    a.setAttribute('download', '例行监测汇总.xls');
                                    a.click();
                                } else {
