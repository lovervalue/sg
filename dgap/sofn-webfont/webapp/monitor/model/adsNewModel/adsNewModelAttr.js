...
myapp.controller("adsNewModelAttr", function ($scope, $http, $state) {
    ...
    window.delAdsCheckModel = function () {
        ...
        for (var i = 0; i < rows.length; i++) {
            var isEnable = rows[i].isEnable;
            if (isEnable == '1'){
                count ++;
            }
        }
        if (count > 0){
            jBox.tip("已经启用的模型配置不能删除，请重新选择!", 'info');
            return false;
        }
        if (check.length > 0) {
            var arr = new Array();
        ...
        $("#itemGrid").dataTable({
            ...
            ajax: {
                ...
                data: function (params) {
                    ...
                    params.token = token;
                    params.industry = $("#check_item_industry").val();
                },
                type: "post"
            },
            columns: [
            ...
            columns: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        for (var i = 0; i < $scope.checkPackages.length; i++) {
                            if ($scope.checkPackages[i] != undefined && row.id == $scope.checkPackages[i]) {
                                return '<input type="checkbox" checked = "true" name="package" onclick="checkSelectThisBox(this)" value="' + data + '"/>';
                            }
                        }
                        return view;
                    }
