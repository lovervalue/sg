...
myapp.controller("specialMonitorList",function($scope,$http,$state){
...
//获取本地存储的机构名称
    ...
    $scope.routineMonitorList = function(data) {
        window.localStorage.setItem("organTaskId",organTaskId);//机构任务ID
        $state.go("index.content.monitor/execUnit/specialMonitor/specialMonitorList")
    }
                    ...
                    if($('#sheng1 option:selected').val()!=null){
                        if($('#shi1 option:selected').val()!=null){
                            if($('#xian1 option:selected').val()!=null){
                                params.producingArea=$('#xian1 option:selected').val();
                            }else{
                                params.producingArea=$('#shi1 option:selected').val();
                            }
                        }else{
                            params.producingArea=$('#sheng1 option:selected').val();
                        }
                    }
                },
                type:"post"
            ...
            columns : [
                ...
                },
                {
                    ...
                    render : function(data, type, row,meta) { // 模板化列显示内容
                        return '<input style="overflow: auto; overflow-y:hidden" readonly="readonly" value="'+data+'" type="text"/>';
                    }
...
//导出excel
    ...
    window.lookOrganRegion=function(data,data1,data2){
        ...
        $state.go("index.content.monitor/leadUnit/AdsMonitorTask/adsCheckInfo/sampleList");
    };
