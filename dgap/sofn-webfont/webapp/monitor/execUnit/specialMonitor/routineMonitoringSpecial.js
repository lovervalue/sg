...
myapp.controller("routineMonitoringSpecial",function($scope,$http,$state){
            ...
            columns : [
                ...
                },
                {
                    ...
                    render : function(data, type, row) {
                        var datax = row.id;
                        var token = window.localStorage.getItem("token");
                        var url = "/sofn-ads-web/adsOrganTask/getOrganStatus?time="+new Date();
                        $http({
                            url: url,
                            method: "post",
                            ...
                            if(0 == status){
                                ...
                                var Row2 = "";
                                Row2 = '<span id="'+row.id+'" class="pull-left task-new">New</span>';
                                $("#"+datax+"").html(Row2);
                                // node.remove();
                            }
                        }).error(function () {
                        });
                        return '<span id="'+row.id+'"></span>'+'&nbsp;&nbsp;'+data;
                    }
                },
                {
                    data : "year",
                ...
                },
                {
                    ...
                    render : function(data, type, row) {
                        if(data!=null && data.length>30){
                            var look_a = '<a id="RMNAME'+row.id+'" onmouseover="showPopContent('+"'"+"RMNAME"+row.id+"'"+","+"'"+data+"'"+')" style="margin-right: 10px;cursor: pointer" onclick="goto('+"'"+row.id+"'"+","+"'"+row.publishStatus+"'"+","+"'RMNAME"+row.id+"'"+')">'+data.substring(0,29)+"..."+'</a>';
                            return look_a;
                        }else{
                            var look_a = '<a style="margin-right: 10px;cursor: pointer" onclick="goto('+"'"+row.id+"'"+","+"'"+row.publishStatus+"'"+')">'+data+'</a>';
                            return look_a;
                        }
                    }
                },
                ...
                },
                {
                    ...
                    render : function(data, type, row) {
                        ...
                        var token = window.localStorage.getItem("token");
                        var url = "/sofn-ads-web/adsOrganTask/getOrganStatusForB?time="+new Date();
                        $http({
                            url: url,
                            method: "post",
                            params: {"id":datax,"token":token}
                            ...
                            var status = data.data;
                            var node = document.getElementById('status'+datax);
                            if(2 == status){
                                $("#status"+datax).addClass("label label-success").html("执行中").attr("val","2");
                            }else if(3 == status){
                                $("#status"+datax).addClass("label label-default").html("已结束,已完成").attr("val","3");
                            }else if(4 == status){
                                $("#status"+datax).addClass("label label-warning").html("已结束,未完成").attr("val","4");
                            }else if(5 == status){
                                $("#status"+datax).addClass("label label-danger").html("已废止").attr("val","5");
                            }else{
                                $("#status"+datax).addClass("label label-primary").html("已发布");
                            }
                        }).error(function () {
                        });
                        return '<span id="status'+datax+'" style="line-height:2;" ></span>';
                    }
                }
            ],
       ...
       /* var url = "/sofn-ads-web/adsMonitorTask/getHangye?time="+new Date();*/
        ...
        selectHangYe.onchange = function(){
            ...
            if("全部"!=hangye){
                ...
                var lihtml = '<li style="margin: 10px;float: left" id="hli">'+'监测行业：'+hangye+'<button class="gui-btn" id="hangye" type="button"><span class="glyphicon glyphicon-remove"></span></button></li>';
        ...
        selectLevel.onchange = function(){
            ...
            if("全部"!=level){
                ...
                var lihtml = '<li style="margin: 10px;float: left" id="eli">'+'级别：'+level+'<button class="gui-btn" id="level" type="button"><span class="glyphicon glyphicon-remove"></span></button></li>';
                var lilength =  $(".setslist ul li").length;
                if(lilength == 0){
                    $(".setslist ul").prepend(lihtml);
                }else{
    ...
    window.goto = function(id,rowId){
        ...
        window.localStorage.setItem("type","2");//类型
    ...
    //鼠标悬停弹窗显示
    window.hidePopContent=function(id,content){
        $("#"+id).popover("hide");
    }
