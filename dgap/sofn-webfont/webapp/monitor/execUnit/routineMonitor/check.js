...
myapp.controller("check", function($scope, $http, $state) {
    ...
    $(function() {
        //设置导航标题
        $("#daoHangTitle").text(monitorClass);
        //bootstrap表格
        $("#mygrid").dataTable({
            ...
            autoWidth: true,
            ajax: {
                url: "/sofn-ads-web/adsCheckInfo/getTestItem",
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if (" " == data) {
                            return (
                                '<input id="' +
                                meta.row +
                                '" placeholder="ND" type="text" name="wu" class="Sup_contain_section_contain_input" onblur="my123(' +
                                meta.row +
                                "," +
                                row.judgeNum +
                                ",'" + row.id + "'" +
                            ...
                            return (
                                ...
                                '" value="' +
                                data +
                                '" type="text" name="wu" class="Sup_contain_section_contain_input" onblur="my123(' +
                                meta.row +
                                "," +
                                row.judgeNum +
                                ",'" + row.id + "'" +
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if (null == data) {
                            return (
                                ...
                                meta.row +
                                "c" +
                                '" type="text" value="" class="Sup_contain_section_contain_input" )"/>'
                            );
                        } else {
                            return (
                                '<input id="' +
                                ...
                                '" type="text" value="' +
                                data +
                                '" class="Sup_contain_section_contain_input" )"/>'
                            );
                        }
                    }
                },
        ...
        var url =
            "/sofn-ads-web/adsCheckInfo/getNextPrimaryWithInfo?time=" +
            new Date();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#product").empty();
                var Row = "";
                if (myData.chanp==null){
                    Row = "<span>产品编码 " + "" + "</span> ";
                }else {
                    Row = "<span>产品编码 " + myData.chanp + "</span> ";
                }
                $("#product").html(Row);
                ...
                if ("" == myData.after && "" == myData.before) {
                    Row2 =
                        '<div style="float: left;">样品编码 <span><input value="' +
                        myData.yangp +
                        '" type="text" name="myName" class="Sup_contain_section_contain_input gui-input"/></span></div>&nbsp;&nbsp;' +
                        // '<button onclick="find2()">查询</button>&nbsp;&nbsp;'+
                        '<span class="gui-btn"  id="after" disabled="true">上一个样品</span>&nbsp;&nbsp;' +
                        '<span class="gui-btn" type="button"  id="next" disabled="true">下一个样品</span>&nbsp;&nbsp;' +
                        '<span class="gui-btn" type="button" onclick="back()">返回</span>&nbsp;&nbsp;' +
                        '<input class="gui-input" type="hidden" name="bannerid" id="bannerid" value="' +
                        myData.id +
                        '">';
                    ...
                    Row2 =
                        ...
                        '" type="text" name="myName" class="Sup_contain_section_contain_input gui-input"/></span></div>&nbsp;&nbsp;' +
                        // '<button onclick="find2()">查询</button>&nbsp;&nbsp;'+
                        '<span class="gui-btn" type="button" id="after" disabled>上一个样品</span>&nbsp;&nbsp;' +
                        '<span class="gui-btn" type="button" onclick="next(' +
                        "'" +
                        myData.after +
                        "'" +
                        ')" id="next">下一个样品</span>&nbsp;&nbsp;' +
                    ...
                    Row2 =
                        ...
                        '" type="text" name="myName" class="Sup_contain_section_contain_input gui-input"/></span></div>&nbsp;&nbsp;' +
                        // '<button onclick="find2()">查询</button>&nbsp;&nbsp;'+
                        '<span class="gui-btn" type="button" onclick="before(' +
                        "'" +
                        myData.before +
                        "'" +
                        ')" id="after">上一个样品</span>&nbsp;&nbsp;' +
                ...
                $("#myinfo").empty();
                Row3 =
                    '<div style="float: left;"><span>样品名称  ' + '<span id="xxx">' +
                    myData.sampleName + '</span>' +
                    " </span>&nbsp;&nbsp;" +
                    '<span class="glyphicon glyphicon-search"></span>' +
                    '<input class="gui-input" type="text" id="productName" name="productName"  placeholder="检测项目模糊搜索"/></div>' +
                    "&nbsp;&nbsp;" +
                    '<span class="gui-btn" type="button" onclick="look(' +
                    "'" +
                    myData.yangp +
                    "'" +
                    ')" id="search">查看样品详细信息</span>&nbsp;&nbsp;' +
                    // '<button id="all" onclick="allInfo()">查看所有数据</button>'+
                    '<span class="gui-btn" id="set" onclick="uploadInfo(' +
                    myData.checkReport +
                    ')">保存</span>';
                $("#myinfo").html(Row3);
                $("#mygrid")
            ...
            success: function(data) {
                ...
                if ("" == myData.after) {
                    Row2 =
                        ...
                        "'" +
                        ')" id="after">上一个样品</span>&nbsp;&nbsp;' +
                        '<span class="gui-btn" id="next" >下一个样品</span>&nbsp;&nbsp;' +
                        '<span class="gui-btn" type="button" onclick="back()">返回</span>&nbsp;&nbsp;' +
                        '<input class="gui-input" type="hidden" name="bannerid" id="bannerid" value="' +
                        myData.id +
                        '">';
            ...
            success: function(data) {
                ...
                if ("" == myData.before) {
                    Row2 =
                        '<div style="float: left;">样品编码 <span><input value="' +
                        myData.yangp +
                        '" type="text" name="myName" class="Sup_contain_section_contain_input gui-input"/></span></div>&nbsp;&nbsp' +
                        // '<button onclick="find2()">查询</button>&nbsp;&nbsp;'+
                        '<span class="gui-btn" id="after" disabled="true">上一个样品</span>&nbsp;&nbsp;' +
                        '<span class="gui-btn" type="button" onclick="next(' +
                        "'" +
                ...
                Row3 =
                    ...
                    "'" +
                    ')" id="search">查看样品详细信息</span>&nbsp;&nbsp;' +
                    // '<span class="gui-btn" id="all" onclick="allInfo()">查看所有数据</span>'+
                    '<span class="gui-btn" id="set" onclick="uploadInfo(' +
                    myData.checkReport +
                    ')">保存</span>';
                $("#myinfo").html(Row3);
    ...
    window.uploadInfo = function(checkReport) {
        if (checkReport == "1" ||  checkReport == "4") {
    ...
    window.my123 = function (index, judgeNum, id) {
        ...
        if (inputValue <= judgeNum) {
            $("#mygrid tr:eq(" + real + ") td:eq(4)").html("合格");
        } else {
            $("#mygrid tr:eq(" + real + ") td:eq(4)").html("不合格");
        }
    };
    ...
    window.look = function(data) {
        ...
        // var newWin=window.open("#/index/content/monitor/execUnit/AdsOrganTask/sampleDetail");
        //   newWin.focus();
        //  return false;
        //   $state.go("index.content.monitor/execUnit/AdsOrganTask/sampleDetail");
        //给样品ID赋值
        ...
        $http({
            url:
                "/sofn-ads-web/adsMonitorSample/findAdsMonitorSampleById?time=" +
                new Date(),
            method: "post",
            ...
            .success(function(data) {
                ...
                if (data.data.PRODUCING_AREA == null) {
                    ...
                    $scope.producingAreaName = $.fn.Cityname(
                        data.data.PRODUCING_AREA
                    );
                }
                if (
                    data.data.PRODUCE_DATE == null ||
                    data.data.PRODUCE_DATE == "1970-01-01"
                ) {
                    $scope.produceDate = "";
                } else {
    ...
    window.allInfo = function() {
        $state.go(
            "index.content.monitor/execUnit/routineMonitor/routineMonitorList"
        );
    };
    ...
    $scope.historyTaskList = function(data) {
        if (monitorClass == "例行监测") {
            $state.go(
                "index.content.monitor/execUnit/routineMonitor/routineReportUpload"
            );
        } else if (monitorClass == "专项监测") {
            $state.go(
                "index.content.monitor/execUnit/specialMonitor/specialReportUpload"
            );
        }
    };
});
