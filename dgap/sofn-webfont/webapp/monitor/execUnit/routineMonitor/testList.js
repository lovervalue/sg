...
myapp.controller("testList",function($scope,$http,$state){
    ...
    $(function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.data.publishStatus == "5"){
                    $scope.isAborted = true;
                } else{
                    $scope.isAborted = false;
                }

                if("专项监测"==monitorClass){
                    $("#daohang").empty();
                    var Row = '<a href="#" ui-sref="index.content.monitor/execUnit/specialMonitor/routineMonitoringSpecial" id="titleSpecial">专项监测</a>&gt;<span>检测</span>';
                    $("#daohang").html(Row);
                }
                //bootstrap表格
                $("#mygrid").dataTable({
                    ...
                    columns : [
                        ...
                        },
                        {
                            ...
                            render : function(data, type, row) { // 模板化列显示内容
                                ...
                                if(4==row.CHECK_REPORT || 1==row.CHECK_REPORT || $scope.disabled || $scope.isAborted){
                ...
                var Row = "<h4>任务名称:"+myData.taskName+"</h4>";
                $("#product").html(Row);
            },
            fail: function (data) {
                if("专项监测"==monitorClass){
                    $("#daohang").empty();
                    var Row = '<a href="">专项监测</a>&gt;<span>检测</span>';
                    $("#daohang").html(Row);
                }
                $.jBox.tip("失败");
            }
    ...
    function checkIsAborted(callback) {
        var url = "/sofn-ads-web/adsCheckInfo/getTaskNameByOrganTaskId?time="+ (+new Date());
        //加载任务名称
        $.ajax({
            url: url,
    ...
    window.up=function(){
        ...
        var ids = "";
        var map = {traceCode:"",result:"",sampleId:""};
        var mapArray =[];
        var rows = $("#mygrid").DataTable().data();
        var num = 0;
        for (var i = 1; i <=rows.length; i++){
            ...
            if(checkbox.checked){
                ...
                num++;
                if ("0"=== td.RESULT){
                    map.traceCode = td.PRODUCT_TRACEABILITY_CODE;
                    map.result = "0";
                    map.sampleId = td.MONITOR_SAMPLE_ID;
                    mapArray.push(map);
                }
                if("-1" === td.RESULT){
                    alert("暂未检测，不能上报");
        ...
        $.ajax({
            ...
            type: "post",
            dataType: "json",
            data: {"ids":ids,"organTaskId":organTaskId,"token":token,"mapArray":JSON.stringify(mapArray)},
            success: function (data) {
                $.messager.show({
                    title : "提示信息",
                    msg : "上报成功！",
    ...
    window.newPage=function(){
        ...
        $("#someInfo2").empty();
        var Row2 = "";
        Row2 = '<div style="margin: 0px 0px 10px 0px">任务通知时间:<input type="text" id="startTime" class="" name="endTime" readonly="readonly" style="width:140px;" value="'+data+'"></div>';
        $("#someInfo2").html(Row2);
    ...
    window.change=function(id,name,yangp,chanp){
        ...
        var Row = '<div class="col-md-4" style="line-height: 28px"><span>样品名称：'+name+'</span></div>'+
            '<div class="col-md-4" style="line-height: 28px"> <span>样品编码：'+yangp+'</span> </div>';
            // '<div class="col-md-4" style="line-height: 28px" > <span>产品编码：'+chanp+'</span></div>';
       // var Row1 = '<div class="col-md-4" style="line-height: 28px" > <span>产品编码：'+chanp+'</span></div>';
        var Row1 = "";
        if (chanp=="null"){
            //修复产品编码为空时表格显示问题用隐藏值来站位
            chanp = '<span style="visibility: hidden;">1.2.666666666998789788.02010101.20180731105229.001</span>';
    ...
    window.my123=function(index,judgeNum,id){
        ...
        if(inputValue<=judgeNum){
            $("#productList2 tr:eq("+real+") td:eq(4)").html("合格");
        }else{
            $("#productList2 tr:eq("+real+") td:eq(4)").html("不合格");
        }
    }
