...
myapp.controller("routineMonitorListRck",function($scope,$http,$state){

    var taskName = window.localStorage.getItem("taskName");//任务名称
    var taskId = window.localStorage.getItem("adsMonitorTaskId");//任务id
    var orgTaskId;
    /**
     * 收起部分条件选择框
     */
    ...
    $.ajax({
        ...
        type: "post",
        dataType: "json",
        data: {"monitorTaskId":taskId,"token":token,"monitorClass":""},
        success: function (data) {
            orgTaskId = data.adsMonitorTask[0].adsOrganTaskList[0].id;
            //bootstrap表格
            $("#grid").dataTable({
                ajax : {
                    headers:{
                    ...
                    data : function(params) {
                        ...
                        params.checkLink = $("#checkLink").val();    //检测环节
    ...
    $scope.routineMonitorList = function(data) {
        $state.go("index.content.monitor/execUnit/Recheck/RoutineMonitor/routineMonitorList")
    }
    ...
    $(function (){
       var monitorClass=$scope.monitorClass;
        ...
        $http({
            ...
            method:"post",
            params: {"monitorClass": monitorClass,"token":token},
            headers: {
...
/*    $(function() {
    ...
    window.lookOrganRegion=function(data,data1,data2){
        ...
        window.localStorage.setItem("urlSource","初检任务");
