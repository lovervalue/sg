...
myapp.controller("testListAudit",function($scope,$http,$state){
            ...
            columns : [
                ...
                },
                {
                    ...
                    render : function(data, type, row) { // 模板化列显示内容
                        ...
                        if(4==row.CHECK_REPORT || 1==row.CHECK_REPORT){
    ...
    function checkIsAborted() {
        ...
        return $scope.isAborted;
    }

    //检验是否填写限量值,并跳转到检测页面
    function checkIsLimitedValue() {
        var pass = true;
        $.ajax({
            ...
            data: JSON.stringify({"id": organTaskId, "token": token}),
            headers: {
                'token': token
            },
            success: function (data) {
                ...
                for (var i = 0; i < crits.length; i++) {
                    if (crits[i].unit == null || crits[i].unit == "" || crits[i].spperLimit == null || crits[i].spperLimit == "") {
                        pass = false;
                        break;
                    }
                }
    ...
    window.add=function(){
        ...
        /*if(!checkIsLimitedValue()){
            $.jBox.tip("请先配置限量值。");
            return;
        }*/

        $state.go("index.content.monitor/execUnit/Audit/testAudit");
    ...
    window.exportExcel = function () {
        ...
        var submit = function (v, h, f) {
            if (v == true)
                $http({
                    url: "/sofn-ads-web/adsInfoProject/downloadInfoProjectForB",
                    method: "post",
                    dataType: "json",
                    params: {"ids": ids},
                    responseType: 'arraybuffer',
    ...
    $scope.test = function (data) {
        $state.go("index.content.monitor/execUnit/Audit/testListAudit")
    }
