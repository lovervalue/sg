...
myapp.controller("sampleMessageAudit", function ($scope, $http, $state) {
    ...
    if (window.localStorage.getItem("sampleFinishNum") == "null") {
        ...
        $("#grid").dataTable({
            ...
            ajax: {
                ...
                data: function (params) {
                    ...
                    if ($('#productionsTatus option:selected').text() != "全部") {
                        params.productionsTatus = $('#productionsTatus option:selected').text();
                    }
                },
                type: "post"
            },
    ...
    window.checkClick = function (data) {
        ...
        $state.go("index.content.monitor/execUnit/Audit/sampleDetail")
    }
...
//抽样机构下拉选
    var url = "/sofn-ads-web/adsOrganTask/findDetectionOrgan?time=" + new Date();
    $.ajax({
        url: url,
        type: "post",
        dataType: "json",
    ...
    //     success: function (data) {
    //         // var data = data;
    //         var data = window.localStorage.getItem("detectionOrgan");
    //         var Row = '<option>全部</option> ';
    //         // if (data.length > 0) {
    //         //     for (var i = 1; i <= data.length; i++) {
    //         //         var td = data[i - 1];
    ...
    $scope.exportExcel = function () {
        var submit = function (v, h, f) {
            if (v == true)
                $http({
                    url: "/sofn-ads-web/adsMonitorSample/downloadMonitorAuditSample",
                    method: "post",
                    dataType: "json",
                    params: params1,
