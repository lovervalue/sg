...
myapp.controller("routineMonitoringAudit", function ($scope, $http, $state) {
    ...
    $http({
        ...
        var Row = '<option value="ALL">全部</option> ';
        for (var i = 0; i < $scope.industry.length; i++) {
            // if(data == $scope.industry[i].id){
            //     return $scope.industry[i].dictName;
            // }
        ...
        $("#grid").dataTable({
            ajax: {
                ...
                data: function (params) {
                    params.industry = $('#hangye option:selected').val();
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if (row.publishStatus == '1') {
                            return '<span class="pull-left task-new">New</span>' + '&nbsp;&nbsp;' + data;
                        } else {
                            return data;
                        }
                    }
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if (data != null && data.length > 30) {
                            var look_a = '<a id="RMNAME' + row.id + '" onmouseover="showPopContent(' + "'" + "RMNAME" + row.id + "'" + "," + "'" + data + "'" + ')" style="margin-right: 10px;cursor: pointer" onclick="goto(' + "'" + row.id + "'" + "," + "'" + row.publishStatus + "'" + "," + "'RMNAME" + row.id + "'" + ",'" + row.industryId + "'" + ')">' + data.substring(0, 29) + "..." + '</a>';
                            return look_a;
                        } else {
                            var look_a = '<a style="margin-right: 10px;cursor: pointer" onclick="goto(' + "'" + row.id + "'" + "," + "'" + row.publishStatus + "'" + ",null,'" + row.industryId + "'" + ')">' + data + '</a>';
                            return look_a;
                        }
                    }
                },
    ...
    window.goto = function (data, status, rowId, industryId) {
        //修改发布状态为执行中
        $http({
            url: "/sofn-ads-web/adsMonitorTask/queryAndUpdateById",
            method: "POST",
            params: {"id": data, "publishStatus": status}
        }).success(function (data) {
        ...
        for (var i = 0; i < $scope.industry.length; i++) {
            if (industryId == $scope.industry[i].id) {
                window.localStorage.setItem("INDUSTRY", $scope.industry[i].dictName);
            }
        }
