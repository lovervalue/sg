...
myapp.controller("problemFileAudit",function($scope,$http,$state){
            ...
            success: function (data) {
                ...
                for(var i=0;i<monitorKind.length;i++){
                    if($scope.disabled){
                        ...
                        Row = Row+'<div class="text-left pd-20 mg-auto"><form class="form-inline well well-sm"><div class="form-group line-20"><label class="control-task">'+monitorKind[i].dictName+'</label></div><button type="button" class="gui-btn" onclick="moreFile('+"'"+monitorKind[i].dictName+"'"+')">更多</button><button type="button" class="gui-btn" onclick="loadFile('+"'"+monitorKind[i].id+"'"+')">上传</button></form></div>';
                    }
                }
                Row=Row+'</div>';
                $("#list").html(Row);
