...
myapp.controller("reportCheckAreaObj",function($scope,$http,$state){
    ...
    var gridFlag = 0;
    $scope.monitorTaskId = 0;
    /**
     * 查询抽样环节报表数据
     */
    $scope.loadReportData = function (id) {
        $scope.monitorTaskId = id;
        if(gridFlag==0){
            $scope.loadReportTable(id);
        }else{
            $("#grid").dataTable().api().ajax.reload();
        }
    }

    $scope.loadReportTable=function (id) {
        $("#grid").dataTable({
            ajax : {
                headers:{
                    token:window.localStorage.getItem("token")
                },
                url : "/sofn-ads-web/adsReport/getCheckAreaObjReportPageInfo",
                dataSrc:function(json){
                    ...
                    $scope.loadReportCharts(json.data.list);    //加载图表数据
                    return json.data.list;
                },
                data : function(params) {
                    //分页条件查询，不实现，自己写
                    params.monitorTaskId = $scope.monitorTaskId;
                },
                type:"post"
            },
            columns : [
    ...
    $scope.exportExcel = function () {
        ...
        var submit = function (v, h, f) {
            if (v == true){
                ...
                $http({
                    url:"/sofn-ads-web/adsReport/exportCheckAreaObjExcel",
                    method:"post",
                    dataType:"json",
                    ...
                    if(blob.size > 0){
                        ...
                        a.setAttribute('download', taskName + '检测地区对象报表.xls');
                        a.click();
                    }else{
