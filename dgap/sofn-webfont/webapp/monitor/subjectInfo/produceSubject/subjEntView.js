var myapp=angular.module("myapp",[]).controller("subjEntView",function($scope,$http,$stateParams, $state){
    ...
    $http({
        ...
        $http.post('/sofn-ales-web/subjEnt/findSubjEntAllAuthentication',$.param({id:$scope.subjEnt.id}), {
            headers: {
    ...
    function loadGrid() {
        $("#changeGrid").dataTable({
            ajax: {
                url: "/sofn-ales-web/subjEnt/getChangeListByEntityId",
                headers: {
                    token: window.localStorage.getItem("token")
                },
    ...
    $scope.openModal = function (event) {
        ...
        $http.post('/sofn-ales-web/subjEnt/findSubjEntAuthenticationList',$.param({id:$scope.subjEnt.id, type: type}), {
            headers: {
                "Content-Type": "application/x-www-form-urlencoded;charset=utf-8",
                token: window.localStorage.getItem("token")
            }
