 ...
 */
var myapp=angular.module("myapp",[]).controller("etlPunishEnterpriseview",function($scope,$http,$stateParams, $state){
    //获取本地存储的ID
    $scope.subjEntId = window.localStorage.getItem("dELPUNISH_EnterpriseId");
    $scope.subjEnt={};
    $scope.subjEntTemp={};
    //获取生产经营主体信息
    $http({
        ...
        if(data.data==null||data.data==""||data.data==undefined){
            $http({
                ...
                if($scope.subjEntTemp.areaId!=null&&$scope.subjEntTemp.areaId!=""&&$scope.subjEntTemp.areaId!=undefined){
                    $scope.areaId =$.fn.Cityname($scope.subjEntTemp.areaId);
                }
                $("#sevenContent").show();
                //$("#enitContent").show();
            }).error(function(data){
    ...
    $scope.removeLocalStorage=function(){
        window.localStorage.removeItem("dELPUNISH_EnterpriseId");
        window.localStorage.setItem("motaikuangBiaoshi","identification")
