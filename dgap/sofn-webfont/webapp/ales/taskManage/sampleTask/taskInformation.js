...
myapp.controller("taskInformation", function ($scope, $http, $state) {
     ...
     *公共值
     * */
    $scope.ruetntaskId = window.localStorage.getItem("ales_taskmanage_mychecktask_superviseCheckTaskId_key");//任务id
    $scope.plantaskId = window.localStorage.getItem("ales_taskManage_sampleTask_taskId");//任务id
   ...
   // $scope.initInfo = function () {
        var id ="";
         if($scope.plantaskId != undefined) {
            id = $scope.plantaskId;
        }else if($scope.ruetntaskId != undefined){
            id = $scope.ruetntaskId;
        }
    ...
    $scope.syncToAds = function () {
        ...
        var taskend = $scope.checkTask.taskEndTime.replace("-","").replace("-","");
      if(date > taskend){
    ...
    window.cySubmit = function (o) {
        ...
        $state.go("index.content.ales/taskManage/sampleTask/taskSampleList");//行业
    };
    /*下载抽样单*/
    ...
    $scope.retrunDiskTop = function () {
            $state.go("index.content.ales/taskManage/sampleTask/sampleTaskList");
