var myapp = angular.module("myapp", []).controller("alesEntrustDetection", function ($scope, $http, $state) {
    //只读权限控制
    ...
    $http({
        ...
        window.localStorage.setItem("entrustDetectionId_key",data.data)
    }).error(function (msg) {
        console.log(msg);
   ...
   $('#taskNameId').keydown(function (e) {
    ...
    $(function () {
        ...
        $("#grid").dataTable({
            ...
            // },
            ajax: {
                url: "/sofn-ales-web/llesEntrustDetection/list",
                    ...
                    render: function (data, type, row) {
                            ...
                            if(row.STATE =='未发布'){
                                if($scope.entrustDetectionId == ''){
                                    entrid = window.localStorage.getItem("entrustDetectionId_key");
                                }else {
                                    entrid = $scope.entrustDetectionId;
                                }
                                if(row.CREATEBY == entrid ){
                                 return '<a style="cursor: pointer;"onclick="queryInfo(\'' + data + '\')">详情</a> <a style="cursor: pointer;"onclick="upd(\'' + data + '\',\'' + row.STATE + '\')">修改</a> <a style="cursor: pointer;"onclick="entrustDetections(\'' + data + '\',\'' + row.TASKNAME + '\')">配置任务</a>  <a style="cursor: pointer;"onclick="dowLoadReport(\'' + data + '\')">报告</a>';
                                }else {
                            ...
                            }else {//onclick="entrustDetections(\'' + data + '\')">配置任务</a>
    ...
    window.dowLoadReport = function (data) {
        ...
        $state.go("index.content.ales/taskManage/taskReport/alesEntrustDetectionReport")
    };
