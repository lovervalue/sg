...
myapp.controller("resourceDetails", function ($scope, $http, $state) {
    /*加载外部的js*/
...
//清除查询条件
    ...
    $("#resource").dataTable({
        ajax: {
            ...
            url: "/sofn-dgap-web/resourceApplication/listDetails",
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data,type,row) { // 模板化列显示内容
                     return '<a data-toggle="modal1" data-target="#Modal" style="margin-right: 10px;cursor: pointer" id="myId" class="color" onclick="getDetails(\'' + data + '\')">详情</a>'
                }
            }
        ]
    });
    ...
    /*打开模态框*/
    window.getDetails = function (data) {
        $http({
            url: "/sofn-dgap-web/resourceApplication/getDetails",
            data: {
                id: data
            },
            method: "post",
            ...
            $("#details").modal("show");
            $scope.resourceName = data.data.RESOURCENAME;
            $scope.resourceTypeU = data.data.RESOURCETYPE;
            $scope.resourceTypeUName = lookupNameSelect($scope.resourceType, $scope.resourceTypeU);
            if (data.data.CREATE_TIME != null && data.data.CREATE_TIME != "" && data.data.CREATE_TIME != undefined) {
                $scope.appliyDate = new Date(parseInt(data.data.CREATE_TIME)+8*3600*1000).toISOString().slice(0, 10);
            }
            if (data.data.EFFECTIVE_DATE != null && data.data.EFFECTIVE_DATE != "" && data.data.EFFECTIVE_DATE != undefined) {
                $scope.EFFECTIVE_DATE = new Date(parseInt(data.data.EFFECTIVE_DATE)+8*3600*1000).toISOString().slice(0, 10);
            }
            if (data.data.EXPIRE_DATE != null && data.data.EXPIRE_DATE != "" && data.data.EXPIRE_DATE != undefined) {
                $scope.EXPIRE_DATE = new Date(parseInt(data.data.EXPIRE_DATE)+8*3600*1000).toISOString().slice(0, 10);
            }
            $scope.APPROVAL=data.data.APPROVAL;
