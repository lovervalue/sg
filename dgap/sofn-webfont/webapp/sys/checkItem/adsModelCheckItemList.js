...
myapp.controller("adsModelCheckItemList",function($scope,$http,$state){
    ...
    $("#addAdsModelCheckItemList").bootstrapValidator();
    $("#updateAdsModelCheckItemList").bootstrapValidator();
    ...
    window.addCheckItem=function () {
        ...
        //提交表单
        $.ajax({
            url:"/sofn-sys-web/adsModelCheckItem/addAdsModelCheckItem",
            type:"post",
            dataType:"json",
    ...
    $(function() {
        ...
        $("#grid").dataTable({
            ajax : {
                url : "/sofn-sys-web/adsModelCheckItem/getPageInfo",
                dataSrc:function(json){
                    json.recordsTotal=json.data.total;//总个数
                    json.recordsFiltered=json.data.total;
                    json.pageNumber=json.data.pageNum;//页码
    ...
    window.updateCheckItem=function () {
        ...
        //提交
        $.ajax({
            url:"/sofn-sys-web/adsModelCheckItem/updateAdsModelCheckItem",
            type:"post",
            dataType:"json",
    ...
    $scope.batchDelete=function () {
        ...
        if(check.length > 0){
            ...
            var submit = function (v, h, f) {
                if (v == true)
                    $.ajax({
                        url:"/sofn-sys-web/adsModelCheckItem/batchDeleteCheckItem",
                        type:"post",
                        dataType:"json",
