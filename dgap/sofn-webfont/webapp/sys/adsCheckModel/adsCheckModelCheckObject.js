...
myapp.controller("adsCheckModelCheckObject",function($scope,$http,$state){
    var token = window.localStorage.getItem("token");

    //获取本地存储的对象modelId
    var modelId = window.localStorage.getItem("modelId");

    var type = "CHECK_OBJECT";
    /**
     * 分页展示数据
     */
    $(function() {
    ...
    window.deleteAdsModelAttributeMapping = function (id) {
        ...
        // 自定义按钮
        $.jBox.confirm("确认删除该检测对象吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });
    };
    ...
    window.delAdsModelAttributeMapping=function () {
        ...
        if(check.length > 0){
            ...
            // 自定义按钮
            $.jBox.confirm("确认删除检测对象吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });
