...
myapp.controller("adsCheckModelSampleLink",function($scope,$http,$state){
    var token = window.localStorage.getItem("token");

    //获取本地存储的对象modelId
    var modelId = window.localStorage.getItem("modelId");

    var type = "SAMPLE_LINK";
    /**
     * 分页展示数据
     */
    $(function() {
    ...
    $scope.addAdsModelAttributeMapping = function () {
        ...
        if(check.length > 0){
            ...
            // 自定义按钮
            $.jBox.confirm("确认添加抽样环节吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });

        }else{
            jBox.tip("请先选择抽样环节数据", 'info');
        }
    };
    ...
    window.deleteAdsModelAttributeMapping = function (id) {
        ...
        // 自定义按钮
        $.jBox.confirm("确认删除该抽样环节吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });
    };
    ...
    window.delAdsModelAttributeMapping=function () {
        ...
        if(check.length > 0){
            ...
            // 自定义按钮
            $.jBox.confirm("确认删除抽样环节吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });
    ...
    $scope.editJudgeStandard = function () {
        window.localStorage.setItem("modelId",modelId);
        $state.go("index.content.sys/adsCheckModel/adsCheckModelJudgeStandard");
    };
});