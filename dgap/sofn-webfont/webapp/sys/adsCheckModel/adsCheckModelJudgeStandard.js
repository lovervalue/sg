...
myapp.controller("adsCheckModelJudgeStandard",function($scope,$http,$state){
    var token = window.localStorage.getItem("token");

    //获取本地存储的对象modelId
    var modelId = window.localStorage.getItem("modelId");

    var type = "JUDGE_STANDARD";
    /**
     * 分页展示数据
     */
    $(function() {
        ...
        $("#grid").dataTable({
            ...
            autoWidth : false,
            ajax : {
                url : "/sofn-sys-web/adsCheckModel/getPageInfoChecked",
    ...
    $scope.loadCheckStandardData = function () {
        $("#itemGrid").dataTable({
            ...
            autoWidth : false,
            ajax : {
                url : "/sofn-sys-web/adsCheckModel/getPageInfoUnChecked",
                dataSrc:function(json){
                    json.recordsTotal=json.data.total;//总个数
                    json.recordsFiltered=json.data.total;
                    json.pageNumber=json.data.pageNum;//页码
    ...
    $scope.addAdsModelAttributeMapping = function () {
        ...
        if(check.length > 0){
            ...
            var submit = function (v, h, f) {
                if (v == true)
                    $.ajax({
                        url:"/sofn-sys-web/adsCheckModel/addAdsModelAttributeMapping",
                        type:"post",
                        dataType:"json",
            ...
            // 自定义按钮
            $.jBox.confirm("确认添加判定标准吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });

        }else{
            jBox.tip("请先选择判定标准数据", 'info');
        }
    };
    ...
    window.deleteAdsModelAttributeMapping = function (id) {
        ...
        var submit = function (v, h, f) {
            if (v == true)
                $http({
                    url:"/sofn-sys-web/adsCheckModel/deleteAdsModelAttributeMapping",
        ...
        // 自定义按钮
        $.jBox.confirm("确认删除该判定标准吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });
    };
    ...
    window.delAdsModelAttributeMapping=function () {
        ...
        if(check.length > 0){
            ...
            var submit = function (v, h, f) {
                if (v == true)
                    $.ajax({
                        url:"/sofn-sys-web/adsCheckModel/delAdsModelAttributeMapping",
                        type:"post",
                        dataType:"json",
            ...
            // 自定义按钮
            $.jBox.confirm("确认删除判定标准吗？", "操作提示", submit, { buttons: { '确认': true, '取消': false} });
    ...
    $scope.editType = function () {
        ...
        $state.go("index.content.sys/adsCheckModel/adsCheckModelType");
    };

    //跳转到编辑抽样环节的页面
    $scope.editSampleLink = function () {
        window.localStorage.setItem("modelId",modelId);
        $state.go("index.content.sys/adsCheckModel/adsCheckModelSampleLink");
    };

    //跳转到编辑检测对象的页面
    $scope.editCheckObject = function () {
        window.localStorage.setItem("modelId",modelId);
        $state.go("index.content.sys/adsCheckModel/adsCheckModelCheckObject");
    };
});