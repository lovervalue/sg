...
myapp.controller("testPesticideResiduesList", function($scope, $http, $compile, $state) {
    ...
    $scope.addModal = function() {
        ...
        $scope.addUpdateModal.standardCodeDisable = false;
    ...
    window.update = function(id) {
        ...
        $http({
            ...
            if (json.httpCode == "200") {
                ...
                $scope.addUpdateModal.standardCodeDisable = true;
                $scope.addUpdateModal.testItemScope = json.data.scope;
                $("#pr_add_modal").modal("show");
            } else {
                alert(json.msg);
            }
    ...
    // 根据选定检测项里面的行业分类查找农产品代码表里面使用的行业分类
    var findProductType = function(productTypes, scope) {
        return productTypes.filter(function(e) {
            return e.name == scope;
        });
    }
    // 打开检查对象选择模态框
    $scope.openObjectsSelect = function() {
        ...
        var productTypeOfSelectedTestItem = findProductType($scope.testObjectsModal.productTypes, $scope.testItemsModal.selectedData.scope);
        if (productTypeOfSelectedTestItem.length == 0) {
            var scope = $scope.addUpdateModal.testItemScope == null ? "" : $scope.addUpdateModal.testItemScope;
            if (scope == "") {
                $scope.testObjectsModal.typeId = "";
            } else {
                var productTypes = findProductType($scope.testObjectsModal.productTypes, scope);
                if (productTypes.length == 0) {
                    $scope.testObjectsModal.typeId = "";
                } else {
                    $scope.testObjectsModal.typeId = productTypes[0].value;
                }
            }
            $scope.loadTestObjects(1);
            $("#objects_select_modal").modal("show");
        } else {
            $scope.testObjectsModal.typeId = productTypeOfSelectedTestItem[0].value;
            $scope.loadTestObjects(1);
            $("#objects_select_modal").modal("show");
        }
