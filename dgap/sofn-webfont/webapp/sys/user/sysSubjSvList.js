...
myapp.controller("sysSubjSvList",function($scope,$http,$state){
    // 菜单权限
    ...
    function loadGrid() {
        $("#svGrid").dataTable({
            ajax: {
                url: "/sofn-asms-web/subjSupervise/getSubjSuperviseListForSys",
    ...
    window.subjSvView=function(data){
        ...
        $state.go("index.content.sys/user/sysSubjSvView",  {source:'sys'});
    }
    //跳转到监管机构主体撤销申请页面
