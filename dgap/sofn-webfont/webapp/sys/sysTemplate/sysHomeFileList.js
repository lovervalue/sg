...
myapp.controller("sysHomeFileList", function($scope, $http, $state) {
    //查询
    ...
    // 菜单权限
    $scope.menuAuthority = getAuthorityByMenuName('文件管理');
    // 菜单读写权限值
    $scope.readWriteAuthority = MENU_READWRITE_AUTHORITY;
    //新增
    $scope.listAdd = function() {
        $scope.templeName = "";
        $scope.sysTemplate.platform = ""; // 适用平台
        $scope.sysTemplate.category = ""; // 文件分类
        $scope.sysTemplate.templateName = "";
        $scope.sysTemplate.templateType = "";
        $scope.sysTemplate.remark = "";
        $scope.sysTemplate.templateFilename = ""
        $scope.sysTemplate.fileAddress = "";
        $scope.sysTemplate.flags = ""  // 适用系统
    ...
    function settingEnabledIcon(id, status) {
        if (status == "Y") {
            return '<img src="assets/imgs/icon/sys-on.png" status="' + status + '" id="' + id + '"  class="xxx" onclick="changeStatus(this, event)"/>';
        } else {
            return '<img src="assets/imgs/icon/sys-off.png" status="' + status + '" id="' + id + '"  class="xxx" onclick="changeStatus(this, event)"/>';
        }
    }
        ...
        $("#grid").dataTable({
            ajax: {
                ...
                data: function (params) {
                    params.templateName = $.trim($scope.queryCondition.templateName);
                    params.flags = $scope.queryCondition.system;
                    params.platform = $scope.queryCondition.platform;
                    params.category = $scope.queryCondition.category;
                },
                type: "post"
            },
            columns: [{
    ...
    window.changeStatus = function(obj, e) {
        //如果提供了事件对象，
