var myapp = angular
    .module("myapp", [])
    .controller("qryEntrustDetection", function($scope, $http, $state) {
        //判定页面权限、tab页、按钮
        ...
        $(function() {
            $("#grid").dataTable({
                ...
                scrollX: true, //横向滚动
                ajax: {
                    url: "/sofn-qry-web/alesEntrustDetection/list_two",
                    ...
                    data: function(params) {
                        ...
                        params.stUnitName = formatParams($scope.stUnitName);
                        //排序传参
                        if (params.order[0]) {
                    ...
                    },*/
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            return (
                                ...
                                data +
                                '\')">详情</a>&nbsp;&nbsp;&nbsp;&nbsp;<a style="cursor: pointer;" onclick="dowLoadReport(\'' + data + '\',\'' + row.STATE + '\')\">报告</a>'
                            );
                            //}
                        },
                        width: "5%",
     ...
     */
         ...
         *报告下载
         * */
        window.dowLoadReport = function(data,state) {
            window.localStorage.removeItem("qry_wtTaskInfo_ID_Report");
            window.localStorage.removeItem("qry_wtTaskInfo_ID_Report_state");
            window.localStorage.setItem("qry_wtTaskInfo_ID_Report", data);
            window.localStorage.setItem("qry_wtTaskInfo_ID_Report_state", state);
            $state.go(
                "index.content.qry/taskManage/taskReport/qryEntrustDetectionReport"
            );
        };
        //配置任务
        ...
        $scope.exportData = function() {
            ...
            var stUnitName =  $("input[name='stUnitName']").val();//受检单位
            var form = $("<form>");
            ...
            form.attr(
                "action",
                "/sofn-qry-web/alesEntrustDetection/exportSubjSv?token=" +
                    token +
                    "&taskName=" +
                    taskName +
                    "&beginTime=" +
