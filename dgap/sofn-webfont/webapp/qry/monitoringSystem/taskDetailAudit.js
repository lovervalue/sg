...
myapp.controller("taskDetailAudit", function ($scope, $http, $state) {
    ...
    var monitorClass = window.localStorage.getItem("monitorClass");//监测任务
    ...
    $http({
        ...
        method: "post",
        params: {"token": token, "monitorTaskId": adsMonitorTaskId, "monitorClass": "3"}
    }).success(function (data) {
    ...
    //         }
    //     );
    //     $("#checkTable input[spperLimit='spperLimit']").each(function () {
    ...
    //     $.ajax({
    //         type: 'POST',
    //         data: {"criterions": JSON.stringify($scope._sjUnit),'token':token},
    //         headers:{
    //             'token':token
    //         },
    //         url: '/sofn-asms-web/checkTask/updateSpperLimit',
    //         success: function (data) {
    //             alert("保存成功");
