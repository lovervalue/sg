var myapp=angular.module("myapp",[]).controller("qrySubjEntView",function($scope,$http,$stateParams, $state){
    //获取本地存储的ID
    ...
    function loadGrid() {
        $("#changeGrid").dataTable({
            ajax: {
                url: "/sofn-qry-web/subject/getChangeListByEntId",
        ...
        $("#badRecordGrid").dataTable({
            ajax: {
                url: "/sofn-qry-web/subject/getEntBadRecordList",
                dataSrc: function (json) {
                    json.recordsTotal = json.page.recordsTotal;//总个数
                    json.recordsFiltered = json.page.recordsFiltered;
                    json.start = json.page.start;//起始位置
    ...
    $scope.openModal = function (event) {
        ...
        $http.post('/sofn-qry-web/subject/findSubjEntAuthenticationList',$.param({id:$scope.subjEnt.id, type: type}), {
            headers: {
