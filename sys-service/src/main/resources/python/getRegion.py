...
def getVmRegionInfo(province_code, province_name, name, code):
    ...
    parentIds = SPLIT_STR + ROOT_NODE_CODE + SPLIT_STR + province_code + SPLIT_STR + realRegionCode
    parentNames = SPLIT_STR + ROOT_NODE_NAME + SPLIT_STR + province_name + SPLIT_STR + name
    regionInfo = {
        ...
        if len(spanstr) != 0:
            ...
            if spaceNum == 1:
                ...
                if cityCode == '':
                    ...
                    if '县' in regionName:
                        # 应该放置在虚拟区划县中
                        parentIds = SPLIT_STR + ROOT_NODE_CODE + SPLIT_STR + provinceCode + SPLIT_STR + parentIdPrefix + "0200" + SPLIT_STR + regionCode
                        parent_names = SPLIT_STR + ROOT_NODE_NAME + SPLIT_STR + provinceName + SPLIT_STR + '县' + SPLIT_STR + regionName
                        regionInfo = getRegionInfo(regionCode, regionName, parentIdPrefix + '0200', parentIds,
                                                   parent_names)
                        data_list.append(regionInfo)
                    else:
                        # 应该放置在虚拟区划市中
                        parentIds = SPLIT_STR + ROOT_NODE_CODE + SPLIT_STR + provinceCode + SPLIT_STR + parentIdPrefix + "0100" + SPLIT_STR + regionCode
                        parent_names = SPLIT_STR + ROOT_NODE_NAME + SPLIT_STR + provinceName + SPLIT_STR + '市辖区' + SPLIT_STR + regionName
                        regionInfo = getRegionInfo(regionCode, regionName, parentIdPrefix + '0100', parentIds,
                                                   parent_names)
                        data_list.append(regionInfo)
                else:
                    parentIds = SPLIT_STR + ROOT_NODE_CODE + SPLIT_STR + provinceCode + SPLIT_STR + cityCode + SPLIT_STR + regionCode
                    parent_names = SPLIT_STR + ROOT_NODE_NAME + SPLIT_STR + provinceName + SPLIT_STR + cityName + SPLIT_STR + regionName
